...
class InfiniteGridMenu {
  ...
  public run(time = 0): void {
    ...
    // Don't call requestAnimationFrame here - it's handled by the React component
  }
  ...
  private onControlUpdate(deltaTime: number): void {
    ...
    if (!this.control.isPointerDown) {
      ...
      // On first load or when no interaction has occurred, prioritize the first project (JobTrac)
      if (!this._hasUserInteracted && this._frames < 60) { // First second of initialization
        itemIndex = 0; // Always show first project (JobTrac) initially
      }
      ...
      this._hasUserInteracted = true; // Mark that user has interacted
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [], onInitialized }) => {
  ...
  const handleButtonClick = (type: 'github' | 'demo') => {
    ...
    if (type === 'github' && activeItem.github) {
      ...
      // For blogs, use the link property; for projects, use demo
    ...
    <div className="relative w-full h-full">
      ...
      {activeItem && (
        ...
        `}
          ...
          select-none
          absolute
          max-w-[90%] md:max-w-[14ch]
          text-base md:text-[1.1rem]
          leading-snug
          text-gray-600 dark:text-gray-300
          bottom-[20%] md:top-[45%]
          left-1/2 md:left-auto md:right-[5%]
          text-center md:text-left
          ...
          text-shadow-md
          drop-shadow-xl
          [text-shadow:_1px_1px_6px_rgb(0_0_0_/_70%),_-0.5px_-0.5px_2px_rgb(255_255_255_/_15%)]
          dark:[text-shadow:_1px_1px_6px_rgb(0_0_0_/_80%),_-0.5px_-0.5px_2px_rgb(255_255_255_/_25%)]
          ${
            isMoving
              ? 'opacity-0 pointer-events-none duration-[100ms] -translate-x-1/2 md:translate-x-0 md:-translate-y-1/2'
              : 'opacity-100 pointer-events-auto duration-[500ms] -translate-x-1/2 md:translate-x-0 md:-translate-y-1/2'
          }
        `}
          >
            {/* Intelligent truncation based on description length for all devices */}
            {(() => {
              const desc = activeItem.description;
              
              // For very short descriptions, don't truncate
              if (desc.length <= 50) return desc;
              
              // For medium descriptions, use light truncation
              if (desc.length <= 100) {
                return desc.length > 80 ? `${desc.substring(0, 80)}...` : desc;
              }
              
              // For long descriptions, use moderate truncation
              if (desc.length <= 200) {
                return desc.length > 140 ? `${desc.substring(0, 140)}...` : desc;
              }
              
              // For very long descriptions, use standard truncation
          ...
          {/* Action Buttons */}
          {activeItem && (activeItem.github || activeItem.demo || activeItem.link) && (
            <div className={`
              absolute
              left-1/2
              -translate-x-1/2
              ...
              ${
                isMoving
                  ? 'bottom-[-100px] opacity-0 pointer-events-none duration-[100ms] scale-0'
                  : 'bottom-[2em] md:bottom-[3.5em] opacity-100 pointer-events-auto duration-[500ms] scale-100'
              }
            `}>
              {/* GitHub Button - Show if github exists and not a blog */}
              ...
              {/* Demo/Read More Button - Show if demo exists or for blogs */}
              {(activeItem.demo || activeItem.type === 'blogs') && (
                <button
                  ...
                  className="
                    ...
                    hover:shadow-xl
                  "
                  title={activeItem.type === 'blogs' ? 'Read Article' : 'View Live Demo'}
                >
